...
Ранг 5 завершает конечный carrier atlas рукописи. Его носитель имеет \(31\) положение и shell-разложение \(5+10+10+5+1\). В двух средних слоях возникают relation-грамматики на 10-позиционных carriers, включая graph-readings через пересечение и непересечение пар; во внешнем слое появляется residual graph \(K_{2,2,2,2,2}\), cycle \(C_{10}\), проверенное edge-disjoint покрытие \(5C_8\) и projective Singer-cycle \(C_{31}\) через \(\mathbb F_{32}\).
...
\]
Элементы (\operatorname{SubRel}(X,R)) называются подносителями carrier-а ((X,R)).
...
\]
Это корректная presentation с одноточечным волокном identity-reading-а. Recovery datum имеет вид \((\{*\},\varnothing)\).
...
1\mapsto 0.
\]
Так локальный swap получает алгебраическую запись как прибавление единицы в (\mathbb F_2).
...
\[\sigma_i(x)=x+e_i.
\]
Здесь сложение берётся в (\mathbb F_2^n).
...
для единственного \(i\in\{1,2\}\). Следовательно,
...
Пара \(\{00,11\}\) отсутствует в \(\Delta_{\wedge}\). Все три пары его вершин остаются связанными:
...
Пара \(\{00,11\}\) отсутствует в \(\Delta_{\vee}\). Все три пары его вершин остаются связанными:
...
\(\mathfrak C_2\) имеет статус structure/package. В смысле §0.5 presentation имеет форму \((X,R,q,\operatorname{rec})\); \(\mathfrak C_2\) даёт closure-name rank-2 comparison layer-а для пяти уже построенных presentations.
...
На ранге \(2\) mixed-состояния формально составляли punctured carrier \(Q_2^\circ = \{01, 10\}\). На ранге \(3\) это множество перестаёт быть простой двухточечной осью.
...
По Определению 3.5 (§3.9) shell ранга \(n\) задаётся как \(S_k^{(n)} = \{x \in Q_n : |x| = k\}\). Для \(n = 3\):
...
где сложение берётся в \(\mathbb F_2^n\), а \(|x+y|\) означает Hamming-вес. В §6 используется случай \(n=3\).
...
Пара \(\{x,y\}\) принадлежит ровно одному relation \(R_k\), поскольку значение \(d_H(x,y)\) единственно. Значит, отношения \(R_1,R_2,R_3\) попарно не пересекаются и вместе покрывают все пары различных состояний.
...
То есть \(\operatorname{Star}(x)\) есть множество всех камер, содержащих вершину \(x\).
...
множество \(\operatorname{Star}(b_i^\eta)\) читается как coordinate face
...
**Проверка.**
По §8.6 relation \(R_{\operatorname{ch}}\) было задано правилом:
...
Значит, определение через пересечение vertex-support-ов и Hamming-one definition на \(Q_3\) дают одно и то же отношение \(R_{\operatorname{ch}}\).
...
Здесь \(\operatorname{v}\) означает vertex-type, а \(\operatorname{c}\) означает chamber-type.
...
задаётся как симметризация \(\operatorname{Inc}_O\):
...
Поэтому \(T_{\operatorname{ch}}\) корректно определён.
...
Так как \(T\) сохраняет \(R_{12}\), \(T_{\operatorname{ch}}\) сохраняет chamber-adjacency, а вместе они сохраняют incidence, получаем:
...
\hline
T^0 & \operatorname{id} & \text{исходная фаза}\\
...
## §11.15. Difference между \(\tau^2=\operatorname{id}\) и \(T^6=\operatorname{id}\)
...
Отношения \(\rho_1,\rho_2,\rho_3\) являются переносом уже построенных \(R_1,R_2,R_3\) через биекцию \(\operatorname{supp}\). Relation-layer на \(X_{\mathrm{adm}}\) остаётся тем же.
...
**Определение 13.2.**
Vertex/edge form support-reading-а есть чтение \(\operatorname{supp}\) из §12 с разбиением codomain по мощности subset-а:
...
Через support-reading оператор \(\operatorname{opp}\) совпадает с complement-инволюцией \(x\mapsto x+111\). Поэтому \(\operatorname{opp}\) является automorphism support-package-а, в частности automorphism-ом relation-reading-а \((\mathcal P^\circ(J_3),\rho_3)\).
...
# §14. Осевые пары и \(X_{\mathrm{adm}}\cong I_3\times\{-,+\}\)
...
Значит, для каждого \(I_i\in I_3\) и каждого \(\eta\in\{-,+\}\) существует ровно одно состояние
...
## §14.10. \(I_3\times\{-,+\}\) и vertex/opposite-edge
...
В §9 это обозначение использовалось на chamber-coordinate carrier-е \(C_O\cong Q_3\). В §15 используется тот же combinatorial face, но в роли coordinate face полного coordinate carrier-а \(Q_3=\mathbb F_2^3\) из §5. Carrier-роли различны; формула подмножества одна и та же.
...
**Следствие 15.3.**
\(\operatorname{tp}_{2\to3}\) является isomorphism двух two-point pole-carriers с complete two-point relation.
...
## §16.10. Совместимость с \(X_{\mathrm{adm}}\cong I_3\times\{-,+\}\)
...
То есть rank-3 carrier входит в coordinate face \(\{x_D=0\}\) полного rank-4 carrier-а. Например:
...
**Проверка.**
Так как \(Q_4=\mathbb F_2^4\), имеем \(|Q_4|=2^4=16\). Carrier \(\mathcal P_4\) получается удалением \(0000\), поэтому \(|\mathcal P_4|=15\). Carrier \(U_4\) получается удалением двух total poles, поэтому \(|U_4|=14\).
...
Внутри \(\mathcal P_4\setminus\{P_{15}\}\) complement даёт семь пар:
...
Rank \(4\) является even-rank interlayer: его middle shell самодвойственен, а rank-3 carrier входит в coordinate face \(\{D=0\}\) через \(\iota_{3\to4}\).
...
Переход к \(\mathcal P_4=Q_4\setminus\{0000\}\) удаляет edges от \(0000\) к \(S_k^{(4)}\). Таких edges:
...
Для \(\{i\}\in S_1^{(4)}\) и \(Y\in S_3^{(4)}\), если \(Y\neq J_4\setminus\{i\}\), то \(i\in Y\). Тогда:

\[
d_H(\{i\},Y)=|Y|-1=2.
\]

Единственный cross-case distance \(4\) — это complement-pair \(\{i\}\leftrightarrow J_4\setminus\{i\}\), и он исключён из \(\Omega_4\).
...
Каждая \(\Pi_i^{\operatorname{ax},(4)}\) exact: единственное волокно reading-а равно \(H_i^{(4)}\), а recovery возвращает это волокно с induced relation.
...
где \(C_8^{(4)}\) — выбранный Hamilton-cycle на \(V_4\), \(C_6^{(4),\operatorname{mid}}\) — выбранный Hamilton-cycle на middle shell \(S_2^{(4)}\), а \(C_{15}^{(4)}\) — Singer-cycle на \(\mathcal P_4\).
...
Polynomial \(t^4+t+1\) выбран как primitive polynomial. Умножение на \(t\) на \(\mathbb F_{16}^{\times}\) даёт \(15\)-cycle.
...
**Проверка.**
В поле \(\mathbb F_{16}\) группа \(\mathbb F_{16}^{\times}\) имеет \(15\) элементов. Так как \(t^4+t+1\) выбран primitive, class \(t\) порождает \(\mathbb F_{16}^{\times}\). Поэтому последовательность умножений на \(t\) проходит все \(15\) ненулевых элементов и возвращается к исходному через \(15\) шагов. Выписанный cycle является этой последовательностью в bit-basis \(1,t,t^2,t^3\).
...
Для каждой такой middle pair \(H_j^{\operatorname{mid}}\) задаётся presentation:
...
Над \(\mathbb F_2\) все ненулевые векторы уже дают projective points. Поэтому \(P^{(5)}\) допускает readable \(PG(4,2)\)-reading; в §22 native carrier остаётся \(P^{(5)}\).
...
**Проверка.**
Так как \(Q_5=\mathbb F_2^5\), имеем:
...
который лежит вне \(P^{(5)}\). Поэтому active complement-pairs имеют endpoints только в \(P^{(5)}\setminus\{P_{31}\}\).
...
**Утверждение 22.3.**
В \(P^{(5)}\setminus\{P_{31}\}\) complement map \(\kappa_5\) задаёт ровно \(15\) complement-пар. Complement точки \(P_{31}\) равен \(00000\), удалённому из \(P^{(5)}\). Каждая complement-пара задаёт projective line через \(P_{31}\), поэтому таких lines ровно \(15\).
...
**Проверка.**
Если \(x\in S_2^{(5)}\), то его support имеет мощность \(2\). Complement support имеет мощность \(5-2=3\). Поэтому \(\kappa_5(x)\in S_3^{(5)}\). Обратное отображение совпадает с \(\kappa_5\), поскольку \(\kappa_5^2=\operatorname{id}\). Значит, это биекция.
...
Число vertices равно \(\binom52=10\). В graph \(T(5)\) каждая pair \(\{i,j\}\) пересекается по одному element с \(2\cdot3=6\) другими pairs, поэтому:
...
Эти пары покрывают \(V_5=S_1^{(5)}\sqcup S_4^{(5)}\) без пересечений, поэтому \(V_5\cong I_5\times\{-,+\}\). Complement map меняет \(I_i^-\) и \(I_i^+\) внутри каждой пары и не связывает разные пары по \(\mathsf H_5^{(5)}\). Следовательно, graph-reading равен \(5K_2\).
...
выбран как primitive polynomial. Умножение на \(t\) в \(\mathbb F_{32}^{\times}\) даёт \(31\)-cycle на ненулевых vectors.
...
**Проверка.**
Polynomial \(t^5+t^2+1\) выбран как primitive polynomial, поэтому class \(t\) порождает multiplicative group \(\mathbb F_{32}^{\times}\), имеющую порядок \(31\). Следовательно, последовательность умножений на \(t\) проходит все \(31\) ненулевых элементов и возвращается к исходному после \(31\) шагов.
...
В предыдущих разделах каждый rank строился на coordinate carrier-е \(Q_n=\mathbb F_2^n\). Теперь эта запись фиксируется как семейство.
...
**Определение 26.2.**
Для \(\varepsilon\in\mathbb F_2\) и \(x=(x_n,\ldots,x_1)\in Q_n\):
...
Биективность следует из \(\kappa_n^2=\operatorname{id}\).
...
\(\mathfrak R_{\leq5}^{\operatorname{fin}}\) является finite combinatorial package. \(\mathfrak R_{\leq5}\) является closure-name, включающим finite core и finitely specified law-packages.

## §28.2. Что входит в closure

**Утверждение 28.2.**
Документ §§0–25 строит \(\mathfrak R_{\leq5}^{\operatorname{fin}}\) как finite combinatorial package, а §§26–27 строят law-packages \(\mathfrak N\) и \(\mathfrak V\).
...
Эти пять компонентов имеют конечные carriers и конечные relation/presentation/operator данные, поэтому образуют finite package \(\mathfrak R_{\leq5}^{\operatorname{fin}}\).

Общий закон rank-growth \(\mathfrak N\) построен в §26. Universal outer shell \(\mathfrak V\) построен в §27. Они являются law-packages, индексированными по \(n\), и относятся к \(\mathfrak S_{\operatorname{law}}\); finite core содержит только finite carrier-компоненты.
...
остаются доступными для higher ranks, но не являются finite carrier-компонентами \(\mathfrak R_{\leq5}^{\operatorname{fin}}\).
...
Основной результат состоит в том, что эти объекты собираются не как набор отдельных графов, а как согласованная грамматика конечных рангов. Переходы носителей, разложения по слоям, пары дополнения, средние и внешние слои, отношения и циклические чтения имеют общий способ записи и проверки. Ранг \(5\) в этой версии фиксирует носитель \(P^{(5)}=\mathbb F_2^5\setminus\{00000\}\), слоевую структуру \(31=5+10+10+5+1\), двойственность \(S_2^{(5)}\leftrightarrow S_3^{(5)}\), внешний слой \(V_5\cong K_{2,2,2,2,2}\), выбранный \(C_{10}\), проективный цикл \(C_{31}\) и \(5C_8\) как выбранное проверенное покрытие внешнего графа.
