...
- spectral reading of $\mathrm{Res}_{\mathrm{sr}}$;
- modular reading of $\mathrm{Res}_{\mathrm{sr}}$;
...
$$
\text{where on } \mathcal{R}_L \text{ is the maximum of } \mathrm{Res}_{\mathrm{sr}}\text{ attained?}
$$
...
The frontier layer is a superstructure over the residual block $\mathrm{Res}_{\mathrm{sr}}$.
...
$$
\mathrm{frontier\_signature}(P_n)
...
$$
\mathrm{frontier\_signature}(A_n)
\to \text{axial layer}.
$$
...
$$
\mathrm{frontier\_completion\_signature}(\text{paired limit, axial limit})
$$
...
$$
\mathrm{\_assert\_post\_break\_beta\_layer}()
$$

from `DOT_AMR_verifier.py`.

#### 3.3.4. Synchronous Diagonal and the Operator $\Omega_{\mathrm{sync}}$
...
$$
\mathrm{shell}(O_n) = n^2 = A_n.
$$

##### 2. The paired residue equals the axial shell

$$
\mathrm{Res}_{\mathrm{pair}}(n) = n(n-1) = \mathrm{shell}(X_n).
$$
...
$$
\mathrm{ray}(I_n) = 1:n = \mathrm{ray}(X_n).
$$

These coincidences isolate not an arbitrary pair of inputs, but the synchronous diagonal:

$$
\mathfrak{D}_{\mathrm{sync}} = \{ (P_n, A_n) : n \ge 3 \}.
$$

Moreover, it is uniquely selected:

- the equality $\mathrm{shell}(O_n) = A_m$ is possible only when $n = m$;
- the equality $\mathrm{Res}_{\mathrm{pair}}(n) = \mathrm{shell}(X_m)$ is possible only when $n = m$;
- the equality $\gcd(O_n) = \gcd(X_m)$ is possible only when $n = m$;
- the equality $\mathrm{ray}(I_n) = \mathrm{ray}(X_m)$ is possible only when $n = m$.
...
Inside this broad admissible lattice, the synchronous diagonal $\mathfrak{D}_{\mathrm{sync}}$ is a selected exact subfamily, not the whole product.
...
- the synchronous diagonal gives the one-parameter derived operator $\Omega_{\mathrm{sync}}$;
...
- no lift of $\Omega_{\mathrm{sync}}$ into the strict core is built;
...
- the synchronous diagonal and the operator $\Omega_{\mathrm{sync}}$;
